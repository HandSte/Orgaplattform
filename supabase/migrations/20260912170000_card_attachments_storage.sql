...
for insert to authenticated with check (
  bucket_id='card-attachments' and exists (
    select 1 from public.cards c join public.lists l on l.id=c.list_id
    where c.id=((storage.foldername(name))[1])::uuid and public.is_board_member(l.board_id)
  )
);
create policy card_attachments_storage_select on storage.objects
for select to authenticated using (
  bucket_id='card-attachments' and exists (
    select 1 from public.card_attachments a
    join public.cards c on c.id=a.card_id join public.lists l on l.id=c.list_id
...
for delete to authenticated using (
  bucket_id='card-attachments' and (owner_id=auth.uid()::text or exists (
    select 1 from public.card_attachments a join public.cards c on c.id=a.card_id
