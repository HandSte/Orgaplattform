...
export default function CardCollaboration({ supabase, cardId, userId }: Props) {
  ...
  async function addAttachment() {
    ...
    try {
      ...
      Alert.alert('Anhang hinzufügen', error instanceof Error ? error.message : 'Unbekannter Fehler.');
    } finally { setBusy(false); }
  }

  async function openAttachment(attachment: Attachment) {
    const { data, error } = await supabase.storage.from('card-attachments').createSignedUrl(attachment.storage_path, 60 * 10);
  ...
  function removeAttachment(attachment: Attachment) {
    Alert.alert('Anhang löschen', `„${attachment.file_name}“ wirklich löschen?`, [
