...
export default function App() {
  ...
  const [selectedBoard, setSelectedBoard] = useState<string | null>(null); const [loadingBoard, setLoadingBoard] = useState(false); const [boardRole, setBoardRole] = useState<BoardRole | null>(null); const [search, setSearch] = useState('');
  ...
  async function loadBoards() {
    ...
    setSelectedBoard(current => current && next.some(b => b.id === current) ? current : next[0]?.id ?? null);
  }
  ...
  return <SafeAreaView style={styles.safe}><StatusBar style="auto" /><View style={styles.header}><View><Text style={styles.brandSmall}>Essentia</Text><Text style={styles.subtitle}>{activeBoard?.name ?? 'Arbeitsbereich'} · {boardRole === 'viewer' ? 'Nur Lesen' : boardRole ? boardRole : ''}</Text></View><View style={styles.headerActions}><NotificationCenter supabase={supabase} userId={user.id} /><Pressable onPress={() => void signOut()} style={styles.secondary}><Text>Abmelden</Text></Pressable></View></View>
    <View style={styles.quickRow}><Pressable style={styles.quickActive}><Text>Boards</Text></Pressable><Pressable style={styles.quick} onPress={() => Alert.alert('Aufgaben', 'Die zentrale Aufgabenansicht bleibt mit dem Board-Datenmodell verknüpft und wird im nächsten mobilen Navigationsblock ergänzt.') }><Text>Aufgaben</Text></Pressable><Pressable style={styles.quick} onPress={() => Alert.alert('Kalender', 'Kalenderdaten werden weiterhin zentral über Supabase angebunden.') }><Text>Kalender</Text></Pressable><Pressable style={styles.quick} onPress={() => Alert.alert('Dokumente', 'Dokumente bleiben über den gemeinsamen Backend-Kontext erreichbar.') }><Text>Dokumente</Text></Pressable></View>
