...
export default function App() {
  ...
  const [boards, setBoards] = useState<Board[]>([]); const [lists, setLists] = useState<List[]>([]); const [cards, setCards] = useState<Card[]>([]); const [selectedBoard, setSelectedBoard] = useState<string | null>(null); const [loadingBoard, setLoadingBoard] = useState(false); const [boardRole, setBoardRole] = useState<BoardRole | null>(null);
  const [editingCard, setEditingCard] = useState<Card | null>(null); const [cardTitle, setCardTitle] = useState(''); const [cardDescription, setCardDescription] = useState(''); const [cardPriority, setCardPriority] = useState('normal'); const [savingCard, setSavingCard] = useState(false); const [search, setSearch] = useState('');
  ...
  useEffect(() => {
    if (!supabase || !user) { setBoards([]); setSelectedBoard(null); setBoardRole(null); return; }
    void (async () => {
  ...
  useEffect(() => {
    ...
    let alive = true;
    setBoardRole(null);
    void supabase.from('board_members').select('role').eq('board_id', selectedBoard).eq('user_id', user.id).single().then(({ data }) => {
      if (alive) setBoardRole((data?.role as BoardRole | undefined) ?? null);
    });
    return () => { alive = false; };
  }, [selectedBoard, user]);
  ...
  function openNewCard(listId: string) { if (!canEdit) return; setEditingCard({ id: '', list_id: listId, title: '', description: '', position: 999999, priority: 'normal' }); setCardTitle(''); setCardDescription(''); setCardPriority('normal'); }
  async function saveCard() {
    if (!supabase || !editingCard || !cardTitle.trim() || !canEdit) return;
    setSavingCard(true);
    if (editingCard.id) {
  ...
  const filteredCards = (listId: string) => [...cards].filter(x => x.list_id === listId && (!search.trim() || `${x.title} ${x.description ?? ''}`.toLowerCase().includes(search.trim().toLowerCase()))).sort((a,b) => a.position - b.position);
