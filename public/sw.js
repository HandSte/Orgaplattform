const CACHE_NAME = 'orgaplattform-shell-v3';
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(
        keys
          .filter((key) => key.startsWith('orgaplattform-shell-') && key !== CACHE_NAME)
...
self.addEventListener('fetch', (event) => {
  ...
  // Never cache authenticated API traffic or Supabase requests.
