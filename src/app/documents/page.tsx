...
export default function DocumentsPage() {
  const [items, setItems] = useState<(Attachment & { cardTitle: string; boardName: string })[]>([]);
  ...
  async function load() {
    if (!supabase) return;
    setLoading(true);
    const { data, error } = await supabase.from('card_attachments').select('id,card_id,storage_path,file_name,mime_type,size_bytes,created_at').order('created_at', { ascending: false });
    ...
    const boardMap = new Map(((boards ?? []) as BoardRow[]).map(b => [b.id, b.name]));
    setItems(attachments.map(a => { const card = cardMap.get(a.card_id); const list = card ? listMap.get(card.list_id) : undefined; return { ...a, cardTitle: card?.title ?? 'Aufgabe', boardName: list ? boardMap.get(list.board_id) ?? 'Board' : 'Board' }; }));
    setLoading(false);
  }
