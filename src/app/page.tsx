...
export default function Home() {
  ...
  async function loadData() {
    ...
    if (error) setNotice(error.message); else { setBoards(boardData ?? []); setSelectedBoard((current) => current && boardData?.some((b) => b.id === current) ? current : boardData?.[0]?.id ?? null); }
    setLoading(false);
  }
  async function loadBoardContent(boardId: string) {
    ...
    if (ids.length) { const { data } = await supabase.from('cards').select('*').in('list_id', ids).order('position'); setCards(data ?? []); } else setCards([]);
  }
  useEffect(() => { loadData(); if (!supabase) return; const client = supabase; const { data: listener } = client.auth.onAuthStateChange((_event, session) => setUserEmail(session?.user?.email ?? null)); return () => listener.subscription.unsubscribe(); }, []);
  useEffect(() => { if (selectedBoard) loadBoardContent(selectedBoard); }, [selectedBoard]);
  useEffect(() => { if (!supabase || !activeBoard) return; const client = supabase; const channel = client.channel(`board-${activeBoard.id}`).on('postgres_changes', { event: '*', schema: 'public', table: 'lists', filter: `board_id=eq.${activeBoard.id}` }, () => loadBoardContent(activeBoard.id)).on('postgres_changes', { event: '*', schema: 'public', table: 'cards' }, () => loadBoardContent(activeBoard.id)).subscribe(); return () => { client.removeChannel(channel); }; }, [activeBoard?.id]);
  async function createBoard() { if (!supabase) return; const { data: session } = await supabase.auth.getSession(); const user = session.session?.user; if (!user) return (window.location.href = '/auth'); const name = window.prompt('Name des neuen Boards', 'Neues Projekt'); if (!name?.trim()) return; const { data: board, error } = await supabase.from('boards').insert({ name: name.trim(), owner_id: user.id }).select().single(); if (error || !board) return setNotice(error?.message ?? 'Board konnte nicht erstellt werden.'); await supabase.from('board_members').insert({ board_id: board.id, user_id: user.id, role: 'owner' }); for (const [index, title] of ['Backlog', 'In Arbeit', 'Erledigt'].entries()) await supabase.from('lists').insert({ board_id: board.id, name: title, position: index }); await loadData(); setSelectedBoard(board.id); }
  async function addCard(listId: string) { if (!supabase) return; const title = window.prompt('Aufgabe', 'Neue Aufgabe'); if (!title?.trim()) return; const listCards = cards.filter((card) => card.list_id === listId); const position = listCards.length ? Math.max(...listCards.map((card) => card.position)) + 1 : 0; const { data, error } = await supabase.from('cards').insert({ list_id: listId, title: title.trim(), position, priority: 'normal' }).select().single(); if (error) setNotice(error.message); else { setCards((current) => [...current, data as Card]); if (activeBoard) loadBoardContent(activeBoard.id); } }
  ...
  async function moveCard(cardId: string, targetListId: string) { if (!supabase || movingCard) return; const card = cards.find((item) => item.id === cardId); if (!card) return; const targetCards = cards.filter((item) => item.list_id === targetListId && item.id !== cardId); const targetPosition = targetCards.length ? Math.max(...targetCards.map((item) => item.position)) + 1 : 0; if (card.list_id === targetListId) { setDraggedCardId(null); setDragOverListId(null); return; } setMovingCard(true); setCards((current) => current.map((item) => item.id === cardId ? { ...item, list_id: targetListId, position: targetPosition } : item)); const { error } = await supabase.from('cards').update({ list_id: targetListId, position: targetPosition }).eq('id', cardId); if (error) { setNotice(`Aufgabe konnte nicht verschoben werden: ${error.message}`); if (activeBoard) await loadBoardContent(activeBoard.id); } else if (activeBoard) await loadBoardContent(activeBoard.id); setMovingCard(false); setDraggedCardId(null); setDragOverListId(null); }
