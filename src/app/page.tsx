    ...
    <main className="shell">
      ...
      <section className="content" id="dashboard">
        ...
        <section className="board-section" id="boards">
          <div className="section-heading"><div><p className="eyebrow">Aktuelles Board</p><h2>{activeBoard?.name ?? 'Projektübersicht'}</h2></div>{supabaseConfigured && userEmail && boards.length > 1 ? <label className="ghost" style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>Board<select value={activeBoard?.id ?? ''} onChange={(event) => setSelectedBoard(event.target.value)} aria-label="Board auswählen" style={{ border: 0, background: 'transparent', font: 'inherit', outline: 'none' }}>{boards.map((board) => <option key={board.id} value={board.id}>{board.name}</option>)}</select></label> : supabaseConfigured && !userEmail ? <a className="ghost button-link" href="/auth">Board öffnen</a> : null}</div>
