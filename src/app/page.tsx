...
export default function Home() {
  ...
  async function loadChecklistProgress(boardId: string) {
    ...
    const listIds = lists.filter((list) => list.board_id === boardId).map((list) => list.id);
    const cardIds = cards.filter((card) => listIds.includes(card.list_id)).map((card) => card.id);
    ...
    const next: Record<string, ChecklistProgress> = {};
    for (const row of data ?? []) next[row.card_id] = { total_items: row.total_items, completed_items: row.completed_items };
  ...
  async function loadProfiles(boardId: string) {
    const client = supabase; if (!client) return;
    const { data, error } = await client.from('board_members').select('user_id,profiles!board_members_user_id_fkey(id,full_name,avatar_url)').eq('board_id', boardId);
    if (error) { setNotice(error.message); return; }
    setProfiles(((data ?? []).map((m: any) => m.profiles).filter(Boolean)) as Profile[]);
  }

  async function openNotificationCard(id: string) {
    const client = supabase;
    const card = cards.find((c) => c.id === id);
    if (card) return setSelectedCard(card);
    if (activeBoard && client) {
      await loadBoardContent(activeBoard.id);
      const { data } = await client.from('cards').select('*').eq('id', id).maybeSingle();
      if (data) setSelectedCard(data as Card);
    }
  }
  ...
  useEffect(() => { if (!activeBoard) return; void loadChecklistProgress(activeBoard.id); }, [activeBoard?.id, cards, lists, boardRefreshVersion]);
  ...
  useEffect(() => {
    ...
    const channel = client.channel(`board-${boardId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'lists', filter: `board_id=eq.${boardId}` }, (payload) => {
        const nextList = payload.new as List;
        const oldList = payload.old as List;
        if (payload.eventType === 'INSERT' && nextList?.board_id === boardId) {
          setLists((current) => current.some((item) => item.id === nextList.id) ? current : [...current, nextList]);
        } else if (payload.eventType === 'UPDATE') {
          setLists((current) => {
            const wasInBoard = current.some((item) => item.id === oldList?.id);
            const isInBoard = nextList?.board_id === boardId;
            if (isInBoard) return wasInBoard ? current.map((item) => item.id === nextList.id ? nextList : item) : [...current, nextList];
            return current.filter((item) => item.id !== oldList?.id);
          });
        } else if (payload.eventType === 'DELETE') {
          setLists((current) => current.filter((item) => item.id !== oldList?.id));
        }
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'cards' }, (payload) => {
        const nextCard = payload.new as Card;
        const oldCard = payload.old as Card;
        const currentListIds = new Set(listsRef.current.filter((list) => list.board_id === boardId).map((list) => list.id));
        const nextRelevant = !!nextCard?.list_id && currentListIds.has(nextCard.list_id);
        const oldRelevant = !!oldCard?.list_id && currentListIds.has(oldCard.list_id);
        if (payload.eventType === 'INSERT') {
          if (nextRelevant) setCards((current) => current.some((item) => item.id === nextCard.id) ? current : [...current, nextCard]);
        } else if (payload.eventType === 'UPDATE') {
          setCards((current) => {
            if (nextRelevant) return current.some((item) => item.id === nextCard.id) ? current.map((item) => item.id === nextCard.id ? nextCard : item) : [...current, nextCard];
            if (oldRelevant) return current.filter((item) => item.id !== oldCard.id);
            return current;
          });
        } else if (payload.eventType === 'DELETE' && oldCard?.id) {
          setCards((current) => current.filter((item) => item.id !== oldCard.id));
        }
      })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'card_checklist_items' }, () => setBoardRefreshVersion((v) => v + 1))
      .on('postgres_changes', { event: '*', schema: 'public', table: 'board_members', filter: `board_id=eq.${boardId}` }, () => { void loadProfiles(boardId); })
      .subscribe();
  ...
  async function addCard(listId: string) {
    const client = supabase; if (!client) return;
    const title = window.prompt('Aufgabe', 'Neue Aufgabe'); if (!title?.trim()) return;
    ...
    if (error) setNotice(error.message); else if (data) setCards((current) => current.some((item) => item.id === data.id) ? current : [...current, data as Card]);
  }

  async function addList() {
    const client = supabase; if (!client || !activeBoard) return;
    const name = window.prompt('Name der Liste', 'Neue Liste'); if (!name?.trim()) return;
    ...
    if (error) setNotice(error.message); else if (data) setLists((current) => current.some((item) => item.id === data.id) ? current : [...current, data as List]);
  }

  async function moveCard(cardId: string, targetListId: string, beforeCardId?: string | null) {
    const client = supabase; if (!client || movingCard) return;
    const card = cards.find((item) => item.id === cardId); if (!card || beforeCardId === cardId) return;
    const targetCards = cards.filter((item) => item.list_id === targetListId && item.id !== cardId).sort((a, b) => a.position - b.position);
    if (beforeCardId && targetCards.findIndex((item) => item.id === beforeCardId) < 0) return;
    setMovingCard(true);
    const { data, error } = await client.rpc('move_card', { p_card_id: cardId, p_target_list_id: targetListId, p_before_card_id: beforeCardId ?? null });
    if (error || !data) { setNotice(error?.message ?? 'Aufgabe konnte nicht verschoben werden.'); if (activeBoard) await loadBoardContent(activeBoard.id); }
    else { await loadBoardContent(activeBoard?.id ?? ''); }
