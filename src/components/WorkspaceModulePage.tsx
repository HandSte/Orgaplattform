...
export default function WorkspaceModulePage({ title, eyebrow, active, description, children }: Props) {
  ...
  const [name, setName] = useState('Benutzer');
  const [search, setSearch] = useState('');
  const inputRef = useRef<HTMLInputElement>(null);
  useEffect(() => {
    if (!supabase) return;
    supabase.auth.getSession().then(({ data }) => { const user = data.session?.user; if (user) setEmail(user.email ?? ''); });
  ...
  useEffect(() => { if (!supabase) return; supabase.auth.getUser().then(async ({ data }) => { if (!data.user) return; const { data: profile } = await supabase.from('profiles').select('full_name').eq('id', data.user.id).maybeSingle(); if (profile?.full_name) setName(profile.full_name); }); }, []);
  function submitSearch(event: React.FormEvent) { event.preventDefault(); const q = search.trim(); if (!q) return; window.location.href = `/tasks?search=${encodeURIComponent(q)}`; }
  async function signOut() { if (supabase) await supabase.auth.signOut(); window.location.href = '/auth'; }
