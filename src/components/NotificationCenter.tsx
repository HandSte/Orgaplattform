...
export default function NotificationCenter({ onOpenCard }:{onOpenCard:(id:string)=>void}){
 ...
 async function markRead(id:string){if(!supabase)return;const {error}=await supabase.rpc('mark_notifications_read',{p_notification_ids:[id]});if(error)return;const readAt=new Date().toISOString();setItems(v=>v.map(x=>x.id===id?{...x,read_at:readAt}:x));}
 async function markAll(){if(!supabase||!unread)return;setLoading(true);const {error}=await supabase.rpc('mark_notifications_read');if(!error)setItems(v=>v.map(x=>({...x,read_at:x.read_at??new Date().toISOString()})));setLoading(false)}
